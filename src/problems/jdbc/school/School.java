...
public class School {
    ...
    public void listCourses() {
        ...
        try (Connection conn = DatabaseHelper.getConnection();
            ...
            System.out.println("Error fetching list of courses: " + e);
        }
    }

    public void listGrades(int courseId) {
        String query = "Select grade from Grades where course_id = ?";
        try (Connection conn = DatabaseHelper.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(query);) {
            pstmt.setInt(1, courseId);
            ResultSet resultSet = pstmt.executeQuery();
            while (resultSet.next()) {
                System.out.println(resultSet.getFloat("grade"));
            }
        } catch (SQLException e) {
            System.out.println("Error fetching grades: " + e);
        }
    }
    ...
    public void reportUniqueStudents() {
        String query = "Select distinct student_id from Enrollments";
        try (Connection conn = DatabaseHelper.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(query);) {
            ResultSet resultSet = pstmt.executeQuery();
            while (resultSet.next()) {
    ...
    public void reportAverageScore(int courseId) {
        String query = "Select AVG(grade) as average from Grades where course_id = ?";
        try (Connection conn = DatabaseHelper.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(query);) {
            pstmt.setInt(1, courseId);
            ResultSet resultSet = pstmt.executeQuery();
            while (resultSet.next()) {
                System.out.println(resultSet.getInt("average"));
            }
        } catch (SQLException e) {
            System.out.println("Error fetching average score: " + e);
        }
    }
