...
//            System.out.println("Result = " + Math.round(Double.parseDouble(Objects.requireNonNull(reduceExpression(inputStr))) * 100.0) / 100.0);
        ...
        String result = reduceExpression(inputStr);
        if (result == null){
            return Double.NaN;
        }
        else {
        return (Math.round(Double.parseDouble(reduceExpression(inputStr)) * 100.0) / 100.0);
        }
...
//        } else {
//            return ("Stored values: "+ sizedStack);
//        }
//    }
    ...
    public static String recallAllMemory() {
        if (sizedStack.isEmpty()) {
            ...
            String result = "";
            result = "Stored values: " +result + sizedStack.pop() + ", " + sizedStack.toString().replaceAll("(\\[)", "").replaceAll("(\\])", "");
